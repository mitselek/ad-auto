...
- Repl Crunch holds Big Crunch until replicanti has stayed at cap ("Infinite") with no replicanti-galaxy purchases for `amount` seconds (blank = 10) — i.e. galaxy farming is done. A galaxy buy or any dip below cap restarts the clock. Replicanti persists through crunches, so once stable it keeps crunching every `period` ms. Its companion checkbox (next to the label) is **auto-manage**: while on, the row auto-enables when a new reality starts and auto-disables once Time Study 181 is bought — so it farms replicanti galaxies early in a reality and stops when you commit to TS 181. You can still toggle the row by hand; the next matching event re-asserts.
